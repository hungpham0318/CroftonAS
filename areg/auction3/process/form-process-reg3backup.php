...
if (mysqli_connect_errno())
  ...
  //echo $rtime;
$msdname = mysqli_real_escape_string($con,$_POST['dname']);
$msuid = mysqli_real_escape_string($con,$_POST['uid']);
$msdid = mysqli_real_escape_string($con,$_POST['did']);
$msauction = mysqli_real_escape_string($con,$_POST['auction']);
	...
	$inpYourName=mysqli_real_escape_string($con,$_POST['inpYourName']);
	$inpSDContact= mysqli_real_escape_string($con,$_POST['inpSDContact']);
...
$stmt->execute();
	//echo "Successfully Recorded r_record....line 77  <br>";
...
mysqli_select_db($conn, crofton_cas) or die("Could not connect to Database:".mysqli_error());
	...
	for ($x = 0; $x <= 9; $x++){
	 if(!empty( $_POST['txtVIN'.$j])){
		 $vehEntry = array();
		 $arrVIN =  mysqli_real_escape_string($conn, $_POST['txtVIN'.$j]);
		 $arrYear =  mysqli_real_escape_string($conn, $_POST['txtYear'.$j]);
		 $arrMake =  mysqli_real_escape_string($conn, $_POST['txtMake'.$j]);
		 $arrModel =  mysqli_real_escape_string($conn,$_POST['txtModel'.$j]);
		 $arrColor =  mysqli_real_escape_string($conn,$_POST['txtColor'.$j]);
		 $arrMileage =  mysqli_real_escape_string($conn,$_POST['txtMileage'.$j]);
		 $arrAnon =  mysqli_real_escape_string($conn,$_POST['txtAnon'.$j]);
		 $arrStock =  mysqli_real_escape_string($conn,$_POST['inpvehStock'.$j]);
		 $arrDetail =  mysqli_real_escape_string($conn,$_POST['popDetail'.$j]);
		 $arrTrans =  mysqli_real_escape_string($conn,$_POST['txtTrans'.$j]);
...
$insert=mysqli_query($conn, "INSERT INTO `v_record`(`v_id`, `v_did`, `v_uid`, `v_rid`, `txtVIN`, `txtYear`, `txtMake`, `txtModel`, `txtColor`,`txtMileage`, `txtAnon`, `inpvehStock`, `popDetail`, `txtTrans`, `txtPrice`, `r_time`) VALUES ('', '$msdid', '$msuid', '$rid', '{$vehEntry[0]}', '{$vehEntry[1]}', '{$vehEntry[2]}',  '{$vehEntry[3]}',  '{$vehEntry[4]}',  '{$vehEntry[5]}',  '{$vehEntry[6]}',  '{$vehEntry[7]}',  '{$vehEntry[8]}',  '{$vehEntry[9]}','{$vehEntry[10]}', '$rtime')") or die("Not Connected failed before line 129 ".mysql_error());

}
mysqli_close($conn);

if ($insert){
//echo "successful inserts in vrecord line 159";
}else{
//echo "vrecord query failed";
} 
...
$to = "suzianne.aol@gmail.com";
$subject = 'user info from '.$_POST['auction'];
$headers = "From: crofton@croftonauctionservices.com" ."\r\n";
$headers .= "MIME-Version: 1.0" . "\r\n";
$headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";

$message = '<html><body>';
$message .= '<table style="width:750px;font-size:18px;border:1px solid black;"><col width="150px"><col width="600px"><tr><td style="border:1px solid black;text-transform:uppercase;">.....</td><td style="border:1px solid black;">...........................................</td></tr>'; 
foreach ($indicesServer as $arg) { 
    if (isset($_SERVER[$arg])) { 
        $message .= '<tr><td style="border:1px solid black;">arg '.$arg.'</td><td style="border:1px solid black;">: ' . $_SERVER[$arg] . '</td></tr>' ; 
    } 
    else { 
       $message .= '<tr><td style="border:1px solid black;"> arg '.$arg.'</td><td style="border:1px solid black;">: Nothing Returned</td></tr>' ; 
    } 
} 
$message .= '<tr><td style="border:1px solid black;">Userid: </td><td style="border:1px solid black;">: ' . $_POST['uid'] . '</td></tr>' ; 
//$message .= '<tr><td style="border:1px solid black;">time: </td><td style="border:1px solid black;">: ' . $rtime . '</td></tr>' ; 

$message .= '<tr><td style="border:1px solid black;">time: </td><td style="border:1px solid black;">: ' . $_POST['$inpYourName'] . '</td></tr>' ; 


$message .= '<tr><td style="border:1px solid black;">Available Width: </td><td style="border:1px solid black;">: ' . $_POST['hidnInptAvailWidth'] . '</td></tr>' ; 
$message .= '<tr><td style="border:1px solid black;">Available Height: </td><td style="border:1px solid black;">: ' . $_POST['hidnInptHeight'] . '</td></tr>' ; 
$message .= '<tr><td style="border:1px solid black;">Msg: </td><td style="border:1px solid black;">: ' . strip_tags($_POST['inpemailtextarea']) . '</td></tr>' ; 
$message .= "Msg: ". strip_tags($_POST['inpemailtextarea']) . "\r\n";
$message .= "</table>";
$message .= "</body></html>";

mail($to, $subject, $message, $headers);
//echo '<br>success mail sent userinfo- Line 386';

$to = "3017067951@txt.att.net,7146548675@messaging.sprintpcs.com";
//$to = "7146548675@messaging.sprintpcs.com";
$subject = "Reg-". strip_tags($_POST['auction']). "\r\n";
$headers = "From: crofton@croftonauctionservices.com \r\n";
...
echo 'csheppard@bscamerica.com, ';
echo 'svest@bscamerica.com, ';
echo 'bpick@bscamerica.com, ';
...
echo '<h3><a href="http://croftonauctionservices.com/regsuccess.php"><span style="color:black;background-color:white;">Finished</span></h3></a></td>';
...
echo "</body></html>";       
           //Redirect('http://croftonauctionservices.com/regsuccess.php', false);
