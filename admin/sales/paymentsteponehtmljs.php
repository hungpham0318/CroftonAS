...
<div style="border:1px normal #440000;width:60%;height:400px;">
<!--<iframe src="/admin/sales/paymentsteponesniphtmljs.php?iid=<?php echo $qiid;?>" style="border-color:#002200;border-width:15px;width:100%; margin:5%;height:70%;"  sandbox="allow-same-origin allow-scripts"></iframe>--></div>
<?php include "paymentsteponesniphtmljs.php";?>
...
<th>icharge_pd</th>
<th>icharge_desc</th>
<th>icharge_ic_amount</th>
<th>icharge_ic_note</th>
<th>icharge_ic_date</th>
...
<th>Recv&apos;d by</th>
<th>i_payments.ip_type</th>
<th>i_payments.ip_note</th>
<th>i_payments.ip_xtra</th>	
<th>iid</th>
<th>idate</th>
<th>itotal</th>
<th>ipdfurl</th>
...
$(document).ready(function() {
 ...
 ajax: "paymentsteponephp.php?iid=<?php echo $qiid.'&ipid='.$qipid;?>",
display: 'envelope',
//display: 'lightbox',
       table: "#example",
       template: '#customForm',
		...
		},*/
{
 	...
 	titleAttr: 'Sets the payment id for master.mpaid and i_charges.ic_paid',
                action: function ( e, dt, node, config ) {
			editor
			.edit(dt.rows({selected:true}).indexes('ic_id'), false)
     			//.val('master.mpaid', <?php echo $qipid;?> )
//.val('i_charges.ic_paid', '+rowData.id'  )			
...
//$('#paymenttableiid').on( 'click', 'tr', function () {
//  var rowData = paymenttableiid.row( this ).data();
  //example.ajax.url( '/paymentsteponephp.php?id='+rowData.id ).load();
 // example.ajax.url( '/paymentsteponephp.php?iid=<?php echo $qiid."&ipid=". $qrowdataid;?> ).load();
//example.ajax.url( '/paymentsteponephp.php?iid=<?php echo $qiid;?>&ipid='+rowData.val(ip_id) ).load();
//} );


$('#paymenttableiid tbody').on( 'click', 'tr', function () {
        $(this).toggleClass('selected');
 var rowData = paymenttableiid.row( this ).data();
example.ajax.url( '/paymentsteponephp.php?iid=<?php echo $qiid;?>&ipid='+rowData.i_payments.ip_id ).load();
    } );
