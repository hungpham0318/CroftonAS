...
<div id="myModal" class="modal">
  ...
  <div class="modal-content">
    ...
    <table><tr><td ><iframe src="/admin/worldview/css/status-key-iframe.php" style="border:none;width:25em;height:250px;"  sandbox="allow-same-origin allow-scripts"></iframe></td><td><iframe src="/admin/worldview/css/substatus-key-iframe.php"   style="border:none;width:25em;height:250px;"  sandbox="allow-same-origin allow-scripts"></iframe></tr></table>
	  ...
	  <p>  The sold rows do not disappear from the 'Recon' view autmatically (but could be).  This may be a task for Friday afternoon, where you select the sold records and edit them and uncheck the On Recon field. This will remove them from the recon view. This is in case there are sold units that should still be on recon after the sale is recorded in our system.  (I&apos;ve done for this week&apos;s sales, already. (Sale-date: June 2, 2017)
<p>You can only select and EDIT 25 records at a time. (I&apos;m looking into the reason for this, but it may just be a function of the amount of data being manipulated in memory(-as opposed to storage).)
<p>The 'Excel' and 'Copy' buttons next to 'Edit' will include the selected rows and visible columns.   They are both handy for sending data into a spreadsheet.  The Excel button produces a NEW WORKBOOk, while the Copy button COPIES to your clipboard and can then be pasted (ctrl-v) into an existing workbook or Google Sheet(s). The 25-row-limit does not apply to copying, nor to the Excel option.  One caveat, however, is that the header row is copied and pasted as well, so allow for that when pasting. (Then just delete the row afterward if you don&apos;t want it in the destination sheet.  (I&apos;m in the process of trying to exclude the header row for copied rows.))
<p>So for the testing period, you might mimic the master layout using 'Column Visibilty' and then copy the desired rows and paste into your Monthly Master -new week sheet.
  ...
  header: true,
   ...
   orientation: 'landscape',
    ...
    exportOptions: {
     ...
     columns: [6,7,8,9,5,17,19,20,21,22,23,24,27,28,29,30,36,37]
