...
Field::inst('master.mpaid'),
	  Field::inst('master.mrepfee'),
Field::inst('master.mrepfeedesc') 
            )
...
->LeftJoin('auctions','auctions.a_id','=','master.maid')
 // ->where('master.mreconview','0','!=')
  ->where('master.mstatus','S','=')
   ->where('master.maid','1','=')
//  ->where('master.msoldprice',0,'=')
//     ->where('master.msalesnet',0,'=')
->where('master.marchive','1','!=')
 ...
 ->where( function ( $q ) {
    $q->where( 'master.msolddate', 'DATE_ADD( NOW(), INTERVAL -90 DAY )', '<=', false );
} )   
//->where('strtotime("master.mrtime")','$d','>=')
    ->process($_POST)
    ->json();
    ?>