...
$qiid= $_GET['iid'];
$qipid= $_GET['ipid'];
$qrowdataid = $_GET['ipid'];
echo 'rowdataid:'. $qrowdataid;
echo 'qiid: '.$qiid.' qipidl: '.$qipid;
...
//->where('master.mpaid',$qipid,'=')
->where( function ( $q ) {
   $q->where( 'master.mid', '(SELECT  ic_mid FROM i_charges WHERE i_charges.ic_mid = master.mid)', 'IN', false );
})
    ->process($_POST)
    ->json();
    ?>