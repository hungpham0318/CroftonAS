...
<table id="example" class="display compact" cellspacing="0" width="95%">
                    <thead>
 ...
 */
 ...
 /*            {
  ...
  //.val('master.msolddate', '<?php echo date("Y-m-d");?>' )
    ...
    {
  text: 'Remove from Recon',
  titleAttr: 'Remove from Recon View',
  action: function(e, dt, node, config) {
    editor
      .edit(dt.rows({
        selected: true
      }).indexes('mid'), false)
      .val('master.reconview', '0')
.submit();},
     		
		},       
{
 	text:   'Push to Arbitration' , 
 	titleAttr: 'Sets Sold Date to today, Status to S, and Substatus to Inv-No. Next, select record(s) and use "Edit button" to add Sold Price, Sale Type, and Sales Net',
			...
			.val('master.mstatus', 'Z' )
			//.val('master.msolddate', '<?php echo date("Y-m-d");?>' )
     			.submit();},
     		
		},
		{
 	text:   'Push to Archive' , 
 	titleAttr: 'Sets Sold Date to today, Status to S, and Substatus to Inv-No. Next, select record(s) and use "Edit button" to add Sold Price, Sale Type, and Sales Net',
                action: function ( e, dt, node, config ) {
			editor
			.edit(dt.rows({selected:true}).indexes('mid'), false)
     			.val('master.marchive', '1' )
