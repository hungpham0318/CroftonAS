...
echo '<pre></br>icharge array'; print_r($ichargeArr); echo '</br></pre>';
echo '<pre></br>master-invoice array'; print_r($mastinvArr); echo '</br></pre>';

echo 'icharge row1' . $ichargeArr[0];
echo 'icharge row2' . $ichargeArr[1];
if(isset($_POST["submit"]))
{

/*-------------------
 View Invoice and user either makes changes or accepts records as shown and clicks submit 
--------------------



echo 'line216 -issetpostsubmit';
$ic_qty = $_POST["ic_qty"];
...
$resultmu = mysqli_query($con, "UPDATE master SET mid = $mid, maid = $maid, muid = $muid, mdid = $mdid, mvin = '$mvin', myear = $myear, mmodel = '$mmodel', mstock = '$mstock', mstatus = '$mstatus', msubstatus = '$msubstatus', msolddate = '$msolddate', msoldprice = '$msoldprice', minvid = $minvid WHERE `mid` = $mmid");

// returns false if something went wrong with the query
if($resultmu === FALSE) { echo 'line 348 master insert failed<br>';
    die(mysqli_error($con));
}
echo 'line 356<br>';
...
$mi_mid = mysqli_real_escape_string($con, $mid);
...
// returns false if something went wrong with the query
if($resultmi === FALSE) { echo 'mi_relate update died at line 381- resultmi';
     // die(mysqli_error($con));
}
echo 'line 402<br>';
//--------------------
...
// returns false if something went wrong with the query
//if($resultui === FALSE) { echo 'died at line 414';
     // die(mysqli_error($con));
//}
echo 'line 417<br>';
//--------------------
...
// returns false if something went wrong with the query
if($resultdi === FALSE) { echo 'died at line 429';
      //die(mysqli_error($con));
}
echo 'line 413<br>';
//---------CREATE IIC ------------------------
