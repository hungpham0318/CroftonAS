...
$result = mysqli_query($con, "SELECT `mid`, `mvid`, `maid`, `muid`, `mdid`, `mvin`, `myear`, `mmodel`, `mstock`, `mstatus`, `msubstatus`, `msolddate`, `minvid`, `did`,`dnumber`,`dname`,`daddress`,`dcity`,`dstate`,`dzip` 
...
$mmid = mysqli_real_escape_string($con, $qmid);
//$resultmu = mysqli_query($con, "REPLACE INTO `master`(`mstatus`, `msubstatus`, `msolddate`, `minvid`,`marchive`) VALUES ('S','inv-yes','$msolddate','$qiid' WHERE `mid` = $qmid");
$resultmu = mysqli_query($con, "UPDATE master SET mstatus='S',msubstatus='inv-open',msolddate='$msolddate', minvid='$minvid', `marchive` = 0 WHERE mid = '$mmid'");
...
//$ui_iid = $iid;
//include "../../process/connecti.php";
//$ui_iid = mysqli_real_escape_string($con, $iid);
//$ui_uid = mysqli_real_escape_string($con, $muid);
//$resultui = mysqli_query($con, "INSERT INTO `ui_relate`(`ui_iid`, `ui_uid`) VALUES ('$ui_iid','$ui_uid')");
//$resultui = mysqli_query($con, "UPDATE ui_relate SET ui_iid='$ui_iid', ui_uid = '$ui_uid'");
// returns false if something went wrong with the query
//if($resultui === FALSE) { 
      //die(mysqli_error($con));
//}
