...
$postiid=mysqli_real_escape_string($con, $_POST['iid']);
...
echo 'stop thinking there is something wrong with the array, if it is empty then ask yourself if it has been posted, dumbass!  It hasnt!';
/*/ ********************************************  
...
$ichargeEntry[13] = $row['msubstatus'];
$ichargeEntry[14] = $row['ic_did'];
$ichargeEntry[15] = $row['mstatus'];
$ichargeEntry[16] = $row['ic_id'];

$ichargeArr[] = $ichargeEntry;
$itotal = $itotal+($row['ic_qty'] * $row['$ic_rate']);
...
include "../../process/connecti.php";
$result3 = mysqli_query($con, $sqllist);
if($result3 != TRUE) { echo " oops at line 423 SELECT FROM master failed!".mysqli_error($con);
}
...
include "../../process/connecti.php";
$result2 = mysqli_query($con, "INSERT INTO `test_invoices_temp` (`iid`, `idate`, `itotal`, `ipdfurl`, `iaid`, `idid`, `idname`, `idattn`, `idaddress`,`idcitystatezip`, `idinvoiceemail`, `iclosed`) VALUES ($iid, $idate, $itotal, '$ipdfurl', '$iaid', $qdid, '$idname', '$idattn', '$idaddress', '$idaddress2', '$idinvoiceemail', 0)");
// mysqli_query returns false if something went wrong with the query
if($result2 != TRUE) { echo "<br />online 362 result2-  the error is ".mysqli_error($con)."<br />";
$result12 = mysqli_query($con, "UPDATE `test_invoices_temp` SET `iid`= $iid,`idate`='$idate',`itotal`=$itotal,`ipdfurl`='$ipdfurl',`iaid`='$iaid',`idname`='$idname',`idattn`='$idattn', `idaddress`='$idaddress', `idcitystatezip`='$idcitystatezip',`idinvoiceemail`='$dinvoiceemail',`iclosed`= 0 WHERE `idid` = $qmdid ");
if($result12 != True) { 
echo '365 '.mysqli_error($con).' didnt update after not inserting<br />';
die(mysqli_error($con));
}}
$ic_iid = $iid;
//$minvid = $iid;
$ipdfurl = "http://www.croftonas.com/admin/accnts/invoice/bunker/preview/$idid.$iid.pdf";
echo '<br />line 371 says: iid = '; 
echo $iid;
echo '<br />line 373 says: url = '.$ipdfurl;
echo '<br />line 373 says: did = '.$idid;
...
// mysqli_query returns false if something went wrong with the query
if($result1 === FALSE) { echo '- 382No vehicles for this dealership are available for invoicing.  This page only returns vehicles whose Status is set to S(old) with the Substatus set to inv-no'.$whereand;
    die(mysqli_error($con));
