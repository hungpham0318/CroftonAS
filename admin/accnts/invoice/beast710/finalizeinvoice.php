...
rename("/preview/'.$idid.'.pdf","/test/bunker/'.$idid.'.'.$iid.'.pdf");
...
$sql2="SELECT sum(ic_amount) as ictotal FROM test_i_charges_temp WHERE ic_did = $qdid";
...
$resultic=mysqli_query($con, "SELECT * FROM test_i_charges_temp WHERE ic_did = $qdid");
...
include "../../process/connecti.php";
$sql3="SELECT * FROM test_invoices_temp WHERE idid = $qmdid";
...
$pdfurl = "test/bunker/".$idid.".".$iid.".pdf";
$ipdfurl = "http://www.croftonas.com/admin/accnts/invoice/$pdfurl";
echo $ipdfurl."<br />";
include "../../process/connecti.php";

$sql4 = "INSERT INTO `test_invoices` (`iid`, `idate`, `itotal`, `ipdfurl`, `iaid`, `idid`, `idname`, `idattn`, `idaddress`,`idcitystatezip`, `idinvoiceemail`, `iclosed`) VALUES (null, '$idate','$ictotal', '$ipdfurl', $iaid, $idid, '$idname', '$idattn', '$idaddress', '$idaddress2', '$idinvoiceemail', $iclosed)";
...
$sql5 = "DELETE FROM `test_i_charges_temp` WHERE ic_did = $qmdid";
...
$sql6 = "DELETE FROM `test_invoices_temp` WHERE idid = $qmdid";
