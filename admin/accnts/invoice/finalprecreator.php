...
include "../../process/connecti.php";
$result4 = mysqli_query($con, "SELECT * FROM invoices_temp WHERE `idid` = $qmdid");
if($result4 === false) {
...
$iclosed= 0;
$ipdfurl = 'https://www.croftonas.com/admin/accnts/invoice/preview/$idid.pdf';
//$itotal = $row['itotal'];
}
...
$mastinvEntry =array();
$whereand = "";
$whereand = 'SELECT * FROM `master` WHERE `mdid` = '.$qdid.' AND `msubstatus` = "Inv-No" AND `mstatus` = "S" ';
...
include "../../process/connecti.php";
$result5 = mysqli_query($con, "SELECT * FROM i_charges_temp WHERE ic_did = $qmdid");
...
//$sqllist= 'SELECT * FROM `master` WHERE `mdid` = '.$qmdid.' AND `msubstatus` = "inv-no" AND `mstatus` = "S"';
// SELECT * FROM `master` WHERE `mdid` = 1023 AND `msubstatus` = "Inv-No" AND `mstatus` = "S" AND `mid` != 12 AND `mid` != 13 AND `mid` != 14 AND `mid` != 15
...
//echo '<br />line 426 says: did = '.$idid;
	//I probably have to delete icharges where ic_did  before inserting new ichargesarray.
	include "../../process/connecti.php";
foreach ($ichargeArr as $ichargeEntry){
//$result1 = mysqli_query($con, "INSERT INTO `i_charges_temp`(`ic_id`, `ic_num`,`ic_iid`, `ic_did`,`ic_mid`, `ic_date`, `ic_amount`, `ic_description`, `ic_solddate`, `ic_qty`, `ic_rate`, `ic_ratedesc`, `ic_maid`, `ic_stock`, `ic_paid`, `ic_note`) VALUES ($ichargeEntry[16],$ichargeEntry[17], $iid,'$ichargeEntry[14]', '$ichargeEntry[2]', '$ichargeEntry[8]', '$ichargeEntry[7]', '$ichargeEntry[0]', '$ichargeEntry[1]', '$ichargeEntry[5]', '$ichargeEntry[6]', '$ichargeEntry[6]', '$ichargeEntry[12]', '$ichargeEntry[3]', '$ic_paid', '$ichargeEntry[8]')");


$result1 = mysqli_query($con, "INSERT INTO `i_charges_temp`(`ic_id`,`ic_num`, `ic_iid`, `ic_mid`, `ic_date`, `ic_amount`, `ic_description`, `ic_solddate`, `ic_qty`, `ic_rate`, `ic_ratedesc`, `ic_maid`, `ic_stock`, `ic_paid`, `ic_note`, `ic_xtraint`, `ic_price`, `ic_did`) VALUES ( $ichargeEntry[16],$ichargeEntry[17], $iid, $ichargeEntry[2], '$ichargeEntry[8]',$ichargeEntry[7], '$ichargeEntry[0]','$ichargeEntry[1]',$ichargeEntry[5], '$ichargeEntry[6]','$ichargeEntry[12]'
,$ichargeEntry[3], '$ichargeEntry[4]', 0, '$ichargeEntry[10]', '0', 0 ,$ichargeEntry[14])");
// mysqli_query returns false if something went wrong with the query
if($result1 == FALSE) {
echo '- line 291 -  records not inserted with  error: '.mysqli_error($con).'this query: '.$whereand.' <br />';
...
if($result != TRUE) {echo "<br />line 302 result-   -- can't get the total from i_charges_temp .the error is ".mysqli_error($con);
}
...
update or replace invoice_temp row where idid = $qmdid before inserting  new or updated invoice info. preserving the open invoice concept but keeping the record updated.-*/
include "../../process/connecti.php";
$result2 = mysqli_query($con, "INSERT INTO `invoices_temp` (`iid`, `idate`, `itotal`, `ipdfurl`, `iaid`, `idid`, `idname`, `idattn`, `idaddress`,`idcitystatezip`, `idinvoiceemail`, `iclosed`) VALUES ($iid, '$idate', '$ictotal', '$ipdfurl', '$iaid', $qdid, '$idname', '$idattn', '$idaddress', '$idcitystatezip', '$idinvoiceemail', 0)");
// mysqli_query returns false if something went wrong with the query
if($result2 === false) { //echo "<br />online 415 result2-  the error is ".mysqli_error($con)."<br />";
$result12 = mysqli_query($con, "UPDATE `invoices_temp` SET `iid`= $iid,`idate`='$idate',`itotal`='$citotal',`ipdfurl`='$ipdfurl',`iaid`='$iaid',`idname`='$idname',`idattn`='$idattn', `idaddress`='$idaddress', `idcitystatezip`='$idcitystatezip',`idinvoiceemail`='$idinvoiceemail',`iclosed`= 0 WHERE `idid` = $qdid ");
if($result12 === false) { 
//echo '418 '.mysqli_error($con).' 418 didnt update after not inserting<br />';
//die(mysqli_error($con));
}}
$URL="/admin/accnts/invoice/finalpreviewstepthree.php?did=$qmdid";
