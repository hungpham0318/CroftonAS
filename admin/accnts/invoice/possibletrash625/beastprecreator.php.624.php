...
//echo ' <br/>line 112 no invoice exists [' . $con->error . ']';
}
...
$mastinvArr =array();
$mastinvEntry =array();
$whereand = 'SELECT * FROM `test_master` WHERE `mdid` = '.$qmdid.' AND `msubstatus` = "Inv-No" AND `mstatus` = "S" ';
...
include "../../process/connecti.php";
$result2 = mysqli_query($con, "INSERT INTO `test_invoices_temp` (`iid`, `idate`, `itotal`, `ipdfurl`, `iaid`, `idid`, `idname`, `idattn`, `idaddress`,`idcitystatezip`, `idinvoiceemail`, `iclosed`) VALUES ($iid, '$idate', '$itotal', '$ipdfurl', '$iaid', $qmdid, '$idname', '$idattn', '$idaddress', '$idcitystatezip', '$idinvoiceemail', 0)");
// mysqli_query returns false if something went wrong with the query
if($result2 != TRUE) { //echo "<br />online 415 result2-  the error is ".mysqli_error($con)."<br />";
$result12 = mysqli_query($con, "UPDATE `test_invoices_temp` SET `iid`= $iid,`idate`='$idate',`itotal`='$itotal',`ipdfurl`='$ipdfurl',`iaid`='$iaid',`idname`='$idname',`idattn`='$idattn', `idaddress`='$idaddress', `idcitystatezip`='$idcitystatezip',`idinvoiceemail`='$idinvoiceemail',`iclosed`= 0 WHERE `idid` = $qmdid ");
if($result12 != True) { 
