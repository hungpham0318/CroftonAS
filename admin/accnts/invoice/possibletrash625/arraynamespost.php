...
$ichargeEntry[16] = mysqli_real_escape_string($con,$_POST['ic_id[]'.$j]);

  $ichargeArr[] = $ichargeEntry;
  $itotalsum = $itotalsum + $_POST['ic_amount'.$j];
  $j++;
} 
echo '<pre>'; print_r($ichargeArr); echo '</pre>';
echo '<pre>'; print_r($ichargeEntry); echo '</pre>';
$numitems = $j;
//echo $numitems;
...
//echo "<script>window.location = 'arraynamesview.php';</script>";
...
$result41 = mysqli_query($con, "UPDATE `test_i_charges_temp` SET 
 ...
 WHERE `ic_did` = $ichargeEntry[14] AND `ic_mid` = $ichargeEntry[2]; ");
...
$result22 = mysqli_query($con, "UPDATE `test_invoices_temp` SET `iid`= $iid,`idate`='$idate',`itotal`= '$itotalsum',`ipdfurl`='$ipdfurl',`iaid`='$iaid',`idname`='$idname',`idattn`='$idattn', `idaddress`='$idaddress', `idcitystatezip`='$idcitystatezip',`idinvoiceemail`='$idinvoiceemail',`iclosed`= 0 WHERE `idid` = $qdid ");
if($result22 != True) { 
...
//echo '<a href = "arraynamesview.php" ><form action="arraynamesview.php"><input name = "did" type = "hidden" value = "'.$idid.'"></form>ended the post part</a>';
header("Location: /admin/accnts/invoice/arraynamesview.php?did=$qmdid");
