    ...
    var a = {
        init: function(c) {
            return this.each(function() {
                ...
                d.inputTemplate = b('<input id="" class="actual_input" type="file">').css({
					display: "none"
                });
                ...
                if (f.itemTemplate == false) {
                    d.queueItem = b('<div class="uploadifive-queue-item tag" data-tag-type="file"><a download title="Download File" class="download_item" href="javascript:void(0)"></a><a title="Remove File" class="close_item" href="javascript:void(0)"></a><div class="inside"><span class="filename"></span><span class="fileinfo"></span><span class="success_tick"></span></div><div class="progress"><div class="progress-bar"></div></div></div>');
                } else {
                ...
                d.addQueueItem = function(j) {
                    if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
						...
						/* PAUL */
						j.queueItem.attr('data-fileupload-id', dataUploadID);
						
						/*$('.uploadifive-queue-item').removeAttr('data-latest-item');
						j.queueItem.attr('data-latest-item', true);*/
						
						var IDtoAssign = f.id + "-file-" + d.fileID++;
						
						j.currentFileID = IDtoAssign;
                        /* PAUL */
						
						j.queueItem.attr("id", IDtoAssign);
                        j.queueItem.find(".close_item").bind("click", function() {
                            a.cancel.call(g, j);
                            return false;
                        });
                        var k = j.name;
                ...
                d.removeQueueItem = function(m, l, k) {
                    ...
                    var j = l ? 0 : 500;
                    if (m.queueItem) {
                        if (m.queueItem.find(".fileinfo").html() != "") {
                ...
                d.checkExists = function(k) {
                    if (b.inArray("onCheck", f.overrideEvents) < 0) {
                        b.ajaxSetup({
                           // async: false causes deprecation notice
                            async: false
                        });
                ...
                d.uploadFile = function(k, l) {
					...
					// Paul: former if (!k.skip && !k.complete && !k.uploading) {
                    if (!k.complete && !k.uploading) {
                        k.uploading = true;
                        d.uploads.current++;
                        d.uploads.attempted++;
                                    ...
                                    if (this.status == 200) {
                                        ...
                                        if (this.status == 404) {
                                            ...
                                            if (this.status == 403) {
                                                ...
                                                d.error("Error - Accepted file formats: PNG, JPEG, GIF, TXT, PDF", k, l);
                                            }
                                        }
                                    }
                                }
                    ...
                    } else { /*alert('uploadFile function could not be called');*/ }
                };
                    ...
                    if (b.inArray("onError", f.overrideEvents) < 0) {
                        switch (l) {
                            ...
                            case "FILE_SIZE_LIMIT_EXCEEDED":
                                ...
                                break;
                            default:
                                errorMsg = "Error - Accepted file formats: PNG, JPEG, GIF, TXT, PDF";
                                break;
                        }
                ...
                d.uploadComplete = function(l, j, k) {
                    if (b.inArray("onUploadComplete", f.overrideEvents) < 0) {
						...
						/* PAUL */
						// return file.currentFileID for onUploadComplete
						j.currentFileID = j.queueItem.attr('id');
						/* PAUL */
                ...
                if (window.File && window.FileList && window.Blob && (window.FileReader || window.FormData)) {
                    ...
                    d.button = b('<label for="" id="' + f.id + '" class="uploadifive-button tipr" data-tip="File" title="Upload PNG, JPEG, GIF, TXT or PDF">' + f.buttonText + "</label>");
                    if (f.buttonClass) {
                    ...
                    if (!f.queueID) {
						...
						/* Default
						d.button.before(d.queueEl);
						*/
						
						/* FOR INVOICE DO THIS:
						var identification = d.button.closest('[data-parent-identifier="one_item_line"]').find('.files_area').attr('data-upload-id');
						$('.' + identification).append(d.queueEl);
						*/
						
						dataUploadID = d.button.find('input[type="file"]').attr('data-fileupload-id');
						
						// add queue item to files area
						$('.files_area[data-fileupload-id="'+dataUploadID+'"]').append(d.queueEl);
								
						// connect label with actual input
						d.button.attr('for', dataUploadID);
						d.button.find('.actual_input').attr('id', dataUploadID);
						/* Paul's Code */
