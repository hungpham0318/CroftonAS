...
echo '<div id="table-scroll"><table id="example" class="display" cellspacing="0"  width="200%" style="overflow-x:scroll;overflow-y:scroll;">';

//echo '<tr><td style="border:none;line-height: 2em;color:#ffffff;">.</td></tr>';
    $sdate = $row[SaleDate];
    $query2 = mysql_query("SELECT r_record.r_id, r_record.r_uid, r_record.r_did, r_record.dname, r_record.inpFiveMilNum, r_record.inpYourEmail, r_record.inpSaleDate, r_record.SaleDate, r_record.inpYourName, r_record.inpSDContact, r_record.inpDPhone, r_record.inpYourPhone, r_record.auction, r_record.rtime, v_record.v_id, v_record.v_did, v_record.v_uid, v_record.v_rid, v_record.txtVIN, v_record.txtYear, v_record.txtMake, v_record.txtModel, v_record.txtColor, v_record.txtMileage, v_record.txtAnon, v_record.inpvehStock, v_record.popDetail, v_record.txtTrans, v_record.txtPrice, v_record.r_time FROM v_record INNER JOIN r_record ON (v_record.v_did = r_record.r_did) AND (v_record.v_uid = r_record.r_uid) AND (v_record.v_rid = r_record.r_id) AND (v_record.r_time = r_record.rtime)  LIMIT 0, 5000");
	
	echo '<thead><tr><th>Sale Date</th><th>Year</th><th>Make</th><th>Model</th><th>Mileage</th><th>Floor</th><th>Color</th><th>VIN</th><th>Detail</th><th>Transport</th><th>Registrant</th><th>Phone</th><th>Email</th><th>Dealer Name</th><th>5MilNum</th><th>DealerPhone</th><th>SaleDayContact</th><th>Announce</th><th>Stock#</th><th>auction</th><th>r_time</th><th>v_rid</th><th>Edit</th></tr></thead>';
	echo '<tfoot><tr><th>Sale Date</th><th>Year</th><th>Make</th><th>Model</th><th>Mileage</th><th>Floor</th><th>Color</th><th>VIN</th><th>Detail</th><th>Transport</th><th>Registrant</th><th>Phone</th><th>Email</th><th>Dealer Name</th><th>5MilNum</th><th>DealerPhone</th><th>SaleDayContact</th><th>Announce</th><th>Stock#</th><th>auction</th><th>r_time</th><th>v_rid</th><th>Edit</th></tr></tfoot><tbody class="display_table">';
	
    while($row = mysql_fetch_array($query2))
   {
    $vehid = $row[v_id];
	echo '<tr><td>'.$row['SaleDate'].'</td><td>'.$row['txtYear'].'</td><td>'.$row['txtMake'].'</td><td>'.$row['txtModel'].'</td><td>'.$row['txtMileage'].'</td><td>'.$row['txtPrice'].'</td><td>'.$row['txtColor'].'</td><td>'.$row['txtVIN'].'</td><td>'.$row['popDetail'].'</td><td>'.$row['txtTrans'].'</td><td>'.$row['inpYourName'].'</td><td>'.$row['inpYourPhone'].'</td><td>'.$row['inpYourEmail'].'</td><td>'.$row['dname'].'</td><td>'.$row['inpFiveMilNum'].'</td><td>'.$row['inpDPhone'].'</td><td>'.$row['inpSDContact'].'</td><td>'.$row['txtAnon'].'</td><td>'.$row['inpvehStock'].'</td><td>'.$row['auction'].'</td><td>'.$row['r_time'].'</td><td>'.$row['r_id'].'</td>';
echo "<td><a href='cas_new/useredit.php?uid=".$row['uid']."'>Edit</a></td></tr>";
