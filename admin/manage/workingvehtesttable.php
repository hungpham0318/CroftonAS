...
//echo '<tr><td style="border:none;line-height: 2em;color:#ffffff;">.</td></tr>';
    $sdate = $row[SaleDate];
     $query2 = mysql_query("SELECT r_record.r_id, r_record.r_uid, r_record.r_did, r_record.dname, r_record.inpFiveMilNum, r_record.inpYourEmail, r_record.inpSaleDate, r_record.SaleDate, r_record.inpYourName, r_record.inpSDContact, r_record.inpDPhone, r_record.inpYourPhone, r_record.auction, r_record.rtime, v_record.v_id, v_record.v_did, v_record.v_uid, v_record.v_rid, v_record.txtVIN, v_record.txtYear, v_record.txtMake, v_record.txtModel, v_record.txtColor, v_record.txtMileage, v_record.txtAnon, v_record.inpvehStock, v_record.popDetail, v_record.txtTrans, v_record.txtPrice, v_record.r_time,v_record.v_status,v_record.v_substatus FROM v_record INNER JOIN r_record ON (v_record.v_did = r_record.r_did) AND (v_record.v_uid = r_record.r_uid) AND (v_record.v_rid = r_record.r_id) AND (v_record.r_time = r_record.rtime) WHERE (`v_id` > 2323 AND `v_uid` != 8) LIMIT 0, 5000");
	
	echo '<thead><tr><th>Sale Date</th><th>Year</th><th>Make</th><th>Model</th><th>Mileage</th><th>Floor</th><th>Color</th><th>VIN</th><th>Detail</th><th>Transport</th><th>Registrant</th><th>Dealership</th><th>Announce</th><th>Stock#</th><th>auction</th><th>status</th><th>substatus</th><th>ViewRegInfo</th><th>EditVehInfo</th></tr></thead>';
	echo '<tfoot><tr><th>Sale Date</th><th>Year</th><th>Make</th><th>Model</th><th>Mileage</th><th>Floor</th><th>Color</th><th>VIN</th><th>Detail</th><th>Transport</th><th>Registrant</th><th>Dealership</th><th>Announce</th><th>Stock#</th><th>auction</th><th>status</th><th>substatus</th><th>ViewRegInfo</th><th>EditVehInfo</th></tr></tfoot><tbody class="display">';
    ...
    while($row = mysql_fetch_array($query2))
   {
	...
	echo '<td><a data-toggle="modal"   class="open-viewInfoModal" href="#viewInfoModal" title="Add this item" data-id="'.$row['v_uid'].'" data-uname="'.$row['inpYourName'].'" data-uphone="'.$row['inpYourPhone'].'" data-uemail="'.$row['inpYourEmail'].'" data-modal-id="popup1">'.$row['inpYourName']."</a></td>";
	
	echo '<td><a data-toggle="modal"  title="Add this item" class="open-viewDInfoModal" href="#viewDInfoModal" data-did="'.$row['v_did'].'" data-dname="'.$row['dname'].'" data-sdcontact="'.$row['inpSDContact'].'" data-dphone="'.$row['inpDPhone'].'" data-modal-id="popup2">'.$row['dname']."</a></td>";
	
	echo '<td>'.$row['txtAnon'].'</td><td>'.$row['inpvehStock'].'</td><td>'.$row['auction'].'</td><td>'.$row['v_status'].'</td><td>'.$row['v_substatus'].'</td>';
...
<input class="nbi" type="hidden" name="uid" id="uid" value=""placeholder="no changes made"></br></br>
...
<div id="viewDInfoModal" class="modal fade">
 ...
 <input type="text" name="dname" id="dname" value=""></br></br>
  ...
  <input class="nbi" type="hidden" name="did" id="did" value=""></br></br>
 <p></p>
