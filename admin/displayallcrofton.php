...
LIMIT 0 , 500");
echo '<table id="testTable" class="display_table"><tr><th>D id</th><th>D Name</th><th>D Phone</th><th>U id</th><th>Name</th><th>Phone</th><th>Email</th></tr><tr>';
 while($row = mysql_fetch_array($result))
    {
    echo '<td>'.$row['did'].'</td>'.'<td>'.$row['dname'].'</td>'.'<td> '.$row['dphone'].' </td>'.'<td> '.$row['uid'].' </td>'.'<td>'." ".$row['uname'].'</td>'."  ".'<td>'.$row['umobile'].'</td>'.'<td>'.$row['uemail'].'</td>'.'</tr>';
    }
...
<br><input type="button" class="pure-button button-success" onclick="tableToExcel('testTable', 'User List')" value="Export to Excel">
