...
//echo "<br>: ".$idnumber;
echo "<br>Dealer Id: ".$idid;
echo "<br>To: ".$idname;
echo "<br>Attn: ".$idattn;
echo "<br>Address: ".$idaddress;
echo "<br>Address2: ".$idaddress2;
//echo 'line 147<br>';
...
if($result3 === FALSE) { 
     die(mysqli_error($con)."<b><br>
     <br>query begins line 179 - This error probably means no record number was sent -or- it cannot find the record that matches the record sent in the url</b>");
    //or die(mysql_error($con));
}
else {
//echo 'line 182<br>';
...
// ----------------------------------------------------------//
// 'insert invoice_temp' query  $result2 
// ----------------------------------------------------------//
include "../process/connecti.php";
//$qmid = mysqli_real_escape_string($con, $_GET['mid']);
$result2 = mysqli_query($con, "INSERT INTO `invoices_temp`(`iid`, `idate`, `itotal`, `ipdfurl`, `iaid`, `idid`, `idname`, `idattn`, `idaddress`,`idcitystatezip`,`iclosed`) VALUES (null, '$idate', '$itotal', '$ipdfurl', $iaid, $idid, '$idname', '$idattn', '$idaddress', '$idaddress2', 0)");

// mysqli_query returns false if something went wrong with the query
if($result2 === FALSE) { echo 'died at line 287 insert invoices query result2 which means there is no iid defined henceforth';
...
$minvid = $iid;
//$iidname= $iid;
$ipdfurl = "http://www.manheimas.com/admin/accnts/invoicepdf/bunker/".$iid.".pdf";
...
//echo 'line 304<br>';
// ----------------------------------------------------------//
// 'insert icharges_temp' query  $result2 
// ----------------------------------------------------------//
	include "../process/connecti.php";
...
if($result4 === FALSE) { echo 'No vehicles for this dealership are available for invoicing.  This page only returns vehicles whose Status is set to S(old) or A(ctive) with the Substatus set to recon-green';
	...
	echo '<a href="viewichargesstep2.php?mdid='.$mdid.'" class="button" >Create Invoice</a>';
	include 'views/ichargestemphtml.html';
