...
if(!$result = $con->query($sql)){
    die('There was an error running the query line 307 [' . $con->error . ']');
}
...
// mysqli_query returns false if something went wrong with the query
if($result2 === FALSE) { echo 'died at line 328 insert invoices query result2 which means there is no iid defined henceforth';
...
if($result4 === FALSE) { echo 'No vehicles for this dealership are available for invoicing.  This page only returns vehicles whose Status is set to S(old) or A(ctive) with the Substatus set to recon-green';
	...
	echo '<a href="/admin/accnts/invoicepdf/index1.php?mdid='.$mdid.'" class="button" >Create Invoice</a>';
