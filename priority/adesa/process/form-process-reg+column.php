...
if (mysqli_connect_errno())
  {
  echo "Failed to connect to MySQL: Line 41 " . mysqli_connect_error();
  }
  //echo $rtime;

$msdname = strtoupper($_POST['dname']);
//echo $msdname."</br>";
$dresults = mysqli_query($con,'SELECT DISTINCT dealers.did,dealers.dnumber,dealers.dname FROM `dealers` WHERE dealers.dname LIKE "$msdname"' );
 while($row = mysqli_fetch_array($dresults)){
//foreach ($dresults as $row)
   $dnumber = $row['dnumber'];
   $did = $row['did'];
 //  echo "line 51 dnumber: ".$dnumber;
//echo "</br>Did= ".$did;
}
//echo "line 51 dnumber: ".$dnumber;
//echo $did;
...
$msauction = mysqli_real_escape_string($con,'ADESA DC');
$maid = mysqli_real_escape_string($con,10);
...
//echo "line66";  
$rid='null';
 $stmt = $con->prepare("INSERT INTO `priority_r_record` (`r_id`,`r_uid`, `r_did`, `dname`, `inpFiveMilNum`, `inpYourEmail`, `inpSaleDate`, `SaleDate`,`inpYourName`, `inpSDContact`, `inpDPhone`, `inpYourPhone`, `auction`, `rtime`,`inpemailtextarea`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
//echo "Successfully Recorded r_record....line 74  <br>";
	$stmt = $con->prepare("SELECT r_id FROM priority_r_record WHERE rtime = '$rtime'");
	$result = $stmt->execute();
...
mysqli_select_db($conn, crofton_cas) or die(" Line 92 Could not connect to Database:".mysqli_error());
	$vehEntry = array();
	$vehTbl=array(); 
	//grab the content
	$j=1;
	...
	for ($x = 0; $x <= 9; $x++){
	 if(!empty( $_POST['txtVIN'.$j])){
		 ...
		 $arrTrans =  mysqli_real_escape_string($conn,strtoupper($_POST['txtTrans'.$j]));
		 	 $arrmcost =  mysqli_real_escape_string($conn,$_POST['mcost'.$j]);
		...
		// $arrvmid= $rid.".".$k;
		 ...
		 $vehEntry[0]= $arrVIN;
			...
			$vehEntry[9] = $arrTrans;
			$vehEntry[10]= $arrmcost;
			$vehEntry[11]= $arrPrice;
			$vehEntry[12]= $arrvmid;
...
$insert=mysqli_query($conn, "INSERT INTO `priority_v_record`(`v_id`, `v_did`, `v_uid`, `v_rid`, `txtVIN`, `txtYear`, `txtMake`, `txtModel`, `txtColor`,`txtMileage`, `txtAnon`, `inpvehStock`, `popDetail`, `txtTrans`, `mcost`,`txtPrice`, `r_time`, `v_status`,`v_substatus`,`vmid`) VALUES ('', '$msdid', '$msuid', '$rid', '{$vehEntry[0]}', '{$vehEntry[1]}', '{$vehEntry[2]}',  '{$vehEntry[3]}',  '{$vehEntry[4]}',  '{$vehEntry[5]}',  '{$vehEntry[6]}',  '{$vehEntry[7]}',  '{$vehEntry[8]}',  '{$vehEntry[9]}','{$vehEntry[10]}','{$vehEntry[11]}', '$rtime','I','recon-red','{$vehEntry[12]}')") or die("Not Connected failed before line 129 ".mysql_error());
}
//mysqli_close($conn);
...
$zdid = $_POST['did'];
include 'msqcon.php';	
$con3=mysqli_connect ($dbhost, $dbuser, $dbpass)or die(" Line 150 Could not connect to MySQL: ".mysqli_error());


mysqli_select_db($con3, crofton_cas) or die("Could not connect to Database: line 245: ".mysqli_error());
//echo $zdid;
$resultsz=mysqli_query($con3, "SELECT drepfee, drepfeedesc FROM `dealers` WHERE did = $zdid");
...
include 'msqcon.php';	
$con2=mysqli_connect ($dbhost, $dbuser, $dbpass)or die(" Line 167 Could not connect to MySQL: ".mysqli_error($con2));
mysqli_select_db($con2, crofton_cas) or die("Could not connect to Database: line 168: ".mysqli_error($con2));
	$masterEntry = array();
	$masterTbl=array(); 
	//grab the content
	$j=1;
	...
	for ($x = 0; $x <= 9; $x++){
	 ...
	 if(!empty( $_POST['txtVIN'.$j])){
		...
		$masterEntry[10]= $arrmcost;
		$masterEntry[11]= $arrmfloor;
		$masterEntry[12]=$arrmvid;
...
//echo 'line 234<pre></br>master table record'; print_r($masterTbl); echo '</br></pre>';
foreach($masterTbl as $masterEntry){ $insertm=mysqli_query($con2, "INSERT INTO `priority_mvehicles`(`mid`,`mvid`,`mrid`,`muid`,`mdid`,`maid`,`mvin`,`myear`,`mmake`,`mmodel`,`mcolor`,`mmileage`,`mannounce`,`mstock`,`mdetail`,`mnotes`,`mcost`,`mfloor`,`mrtime`) VALUES ('', '{$masterEntry[12]}', '$arrmrid','$msuid','$msdid','$maid','{$masterEntry[0]}','{$masterEntry[1]}', '{$masterEntry[2]}','{$masterEntry[3]}','{$masterEntry[4]}', '{$masterEntry[5]}','{$masterEntry[6]}','{$masterEntry[7]}','{$masterEntry[8]}','{$masterEntry[9]}','{$masterEntry[10]}','{$masterEntry[11]}', '$arrmrtime')") ;}	

//foreach($masterTbl as $masterEntry){ $insertm=mysqli_query($con2, "INSERT INTO `priority_master`(`mid`, `mvid`, `maid`, `mrid`, `muid`, `mdid`, `mvin`, `myear`, `mmake`, `mmodel`, `mcolor`, `mmileage`, `mannounce`, `mstock`, `mdetail`, `mtransport`, `mfloor`, `mrtime`, `mreqsaledate2`, `mreqsaledate`, `mstatus`, `msubstatus`, `msolddate`, `mnotes`, `msoldprice`, `mcarfax`, `mdamage`, `mmiscinfo`, `mlane`, `mrun`, `mrundate`, `mrunoutcome`, `minvid`, `marchive`, `mreconview`,`mrepfee`,`mrepfeedesc`) VALUES ('','{$masterEntry[11]}', '$arrmaid', '$arrmrid',  '$msuid',  '$msdid','{$masterEntry[0]}', '{$masterEntry[1]}', '{$masterEntry[2]}', '{$masterEntry[3]}', '{$masterEntry[4]}', '{$masterEntry[5]}', '{$masterEntry[6]}', '{$masterEntry[7]}', '{$masterEntry[8]}', '{$masterEntry[9]}', {$masterEntry[10]}, '$arrmrtime',  '',  '$arrmreqsaledate',  '$arrmstatus',  '$arrmsubstatus', '$arrmsolddate' ,'{$masterEntry[8]}' ,'$arrmsoldprice' ,'$arrmcarfax' ,'$arrmdamage' ,'$arrmmiscinfo' ,'$arrmlane' ,'$arrmrun' ,'$arrmrundate' ,'$arrmrunoutcome', '$arrminvid','$arrmarchive','$arrmreconview',$mrepfee,'$mrepfeedesc')") ;}

//mysqli_close($con2);
//echo '<pre></br>master table record'; print_r($masterTbl); echo '</br></pre>';
if ($insert){
//echo "successful inserts in master line 242";
}else{
echo "master insert query failed";
} 
//$to = "suzanne@croftonas.com,". strip_tags($_POST['inpYourEmail']);
$to = "miked@croftonas.com";
//$to = "priority@croftonas.com";
$subject = "Registration: ".strip_tags($_POST['dname'])." ADESA DC";
$headers = "From: crofton@croftonas.com \r\n";
$headers .= "Reply-To: miked@croftonas.com ". "\r\n";
//$headers .= "Cc:". strip_tags($_POST['inpYourEmail']). "\r\n";
$headers .= 'Cc: russell.stevens@adesa.com,elizabeth.mendoza@adesa.com,richard.andrade@adesa.com,'. strip_tags($_POST['inpYourEmail']) .  "\r\n";
...
$message .='<col width="40px"><col width="151px"><col width="70px"><col width="50px"><col width="110px"><col width="60px"><col width="60px">';
...
$message .= '<td style="width:104px;">NOTES</td>';
$message .= '<td style="width:54px;">COST</td>';
...
$message .= '<td style="width:104px;">' . strip_tags($_POST['mcost'.$j]) . "</td>";
...
//$to = "7146548675@tmomail.net,3017067951@mms.att.net";
$to = "reg@croftonas.com";
$subject = "Reg-".strip_tags($_POST['dname'])." ADESA DC";
...
//$mssge = "By: " . strip_tags($_POST['username']). "\r\n";
...
//echo 'russell.stevens@adesa.com, ';
//echo 'elizabeth.mendoza@adesa.com, ';
...
echo '<table style="table-layout:fixed;border:1px solid black;"><col width="200px"><col width="40px"><col width="70px"><col width="50px"><col width="50px"><col width="40px"><col width="151px"><col width="70px"><col width="50px"><col width="110px"><col width="60px"><col width="60px">';
...
echo '<td style="width:104px;">NOTES</td>';
echo '<td style="width:54px;">COST</td>';
...
echo '<td style="width:54px;">' . strip_tags($_POST['mcost'.$j]) . "</td>";
...
echo "</body></html>";       
           //Redirect('http://croftonas.com/priority/regsuccess.php', false);
