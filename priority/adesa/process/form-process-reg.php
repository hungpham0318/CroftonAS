...
$insert=mysqli_query($conn, "INSERT INTO `priority_v_record`(`v_id`, `v_did`, `v_uid`, `v_rid`, `txtVIN`, `txtYear`, `txtMake`, `txtModel`, `txtColor`,`txtMileage`, `txtAnon`, `inpvehStock`, `popDetail`, `txtTrans`, `txtPrice`, `r_time`, `v_status`,`v_substatus`,`vmid`) VALUES ('', '$msdid', '$msuid', '$rid', '{$vehEntry[0]}', '{$vehEntry[1]}', '{$vehEntry[2]}',  '{$vehEntry[3]}',  '{$vehEntry[4]}',  '{$vehEntry[5]}',  '{$vehEntry[6]}',  '{$vehEntry[7]}',  '{$vehEntry[8]}',  '{$vehEntry[9]}','{$vehEntry[10]}', '$rtime','I','recon-red','{$vehEntry[11]}')") or die("Not Connected failed before line 129 ".mysql_error());
}
//mysqli_close($conn);
...
foreach($masterTbl as $masterEntry){ $insertm=mysqli_query($con2, "INSERT INTO `priority_mvehicles`(`mid`,`mvid`,`mrid`,`muid`,`mdid`,`maid`,`mvin`,`myear`,`mmake`,`mmodel`,`mcolor`,`mmileage`,`mannounce`,`mstock`,`mdetail`,`mnotes`,`mfloor`,`mrtime`) VALUES ('', '{$masterEntry[11]}', '$arrmrid','$msuid','$msdid','$maid','{$masterEntry[0]}','{$masterEntry[1]}', '{$masterEntry[2]}','{$masterEntry[3]}','{$masterEntry[4]}', '{$masterEntry[5]}','{$masterEntry[6]}','{$masterEntry[7]}','{$masterEntry[8]}','{$masterEntry[9]}','{$masterEntry[10]}', '$arrmrtime')") ;}	
...
//$headers .= 'Cc: russell.stevens@adesa.com,elizabeth.mendoza@adesa.com,richard.andrade@adesa.com'. "\r\n";
...
$message .= '<td style="width:54px;">COST</td></tr>';
$j=1;
	for ($x = 0; $x <= 9; $x++){
	 if(!empty( $_POST['txtVIN'.$j])){
...
//$to = "4105706942@messaging.sprintpcs.com,7146548675@tmomail.net";
...
echo '<td style="width:54px;">COST</td></tr>';
