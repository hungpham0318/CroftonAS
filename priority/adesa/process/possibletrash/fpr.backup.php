...
$mauction = $_POST['auction'];
if($mauction = 'Manheim Pennsylvania'){$maid = 1;}elseif($mauction = 'Bel Air'){$maid = 3;}elseif($mauction = 'MANHEIM PA'){$maid=1;}else{$maid=0;}
...
$msauction = mysqli_real_escape_string($con,$mauction);
//$maid = mysqli_real_escape_string($con,$_POST['maid']);
...
mysqli_select_db($conn, manheim_cas) or die("Could not connect to Database:".mysqli_error());
	$vehEntry = array();
	$vehTbl=array(); 
	//grab the content
	$j=1;
...
include '../../common/msqcon.php';	
$con2=mysqli_connect ($dbhost, $dbuser, $dbpass)or die("Could not connect to MySQL: ".mysql_error());
mysqli_select_db($con2, manheim_cas) or die("Could not connect to Database: line 135: ".mysqli_error());
	$masterEntry = array();
	$masterTbl=array(); 
	//grab the content
	$j=1;
	...
	for ($x = 0; $x <= 9; $x++){
	 ...
	 if(!empty( $_POST['txtVIN'.$j])){
		...
		$arrmuid =  $msuid;
		$arrmdid =  $msdid;
		$arrmvin =  mysqli_real_escape_string($con2, $_POST['txtVIN'.$j]);
		$arrmyear =  mysqli_real_escape_string($con2, $_POST['txtYear'.$j]);
		$arrmmake =  mysqli_real_escape_string($con2, $_POST['txtMake'.$j]);
		$arrmmodel =  mysqli_real_escape_string($con2,$_POST['txtModel'.$j]);
		$arrmcolor =  mysqli_real_escape_string($con2,$_POST['txtColor'.$j]);
		$arrmmileage =  mysqli_real_escape_string($con2,$_POST['txtMileage'.$j]);
		$arrmannounce =  mysqli_real_escape_string($con2,$_POST['txtAnon'.$j]);
		$arrmstock =  mysqli_real_escape_string($con2,$_POST['inpvehStock'.$j]);
		$arrmdetail =  mysqli_real_escape_string($con2,$_POST['popDetail'.$j]);
		$arrmtransport =  mysqli_real_escape_string($con2,$_POST['txtTrans'.$j]);
...
$to = "suzanne@suzannebird.com";
$subject = "Registration: ".strip_tags($_POST['dname'])." ".strip_tags($_POST['auction']);
$headers = "From: manhiema@manheimas.com \r\n";
$headers .= "Reply-To: manhiema@manheimas.com ". "\r\n";
...
//$headers = "From: crofton@croftonauctionservices.com \r\n";
$headers = "From: manheim@manheimas.com \r\n";
