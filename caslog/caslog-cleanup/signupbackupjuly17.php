...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $db->prepare('INSERT INTO users (username,password,email,active,uname,umobile,uaddress,ucity,ustate,uzip,ufax, uofficeph,ucompany,uperms) VALUES (:username, :password, :email, :active, :uname, :umobile, :ucompany, :uperms)');


			$stmt = $db->prepare('INSERT INTO INSERT INTO users(username,email,umobile,ufax,uofficeph,username,password,active,uaddress,ucity,ustate,uzip,ucompany,unotes,uperms) VALUES (:uid, :uname, :email, :umobile, :ufax, :uofficeph, :username, :password, :active, :uaddress, :ucity, :ustate, :uzip, :ucompany, :unotes, :uperms)');
			$stmt->execute(array(
...
<tr>
         <td class="sup-label">Address:</td><td><input type="text" class="sup-input" name="uaddress" maxlength="250" value="<?php if(isset($error)){ echo strip_tags($_POST['uaddress']); } ?>" tabindex="3" required></td>
	 <td class="sup-label">Office:</td><td><input pattern="\d{3}[\-]\d{3}[\-]\d{4}" type="tel" title="Enter phone in this format: 000-000-0000"  class="sup-input" name="uofficeph" value="<?php if(isset($error)){ echo strip_tags($_POST['uofficeph']); } ?>"tabindex="4"  required></td>
</tr>
<tr>
	<td class="sup-label">City:</td><td><input type="text" class="sup-input" name="ucity" maxlength="250" value="<?php if(isset($error)){ echo strip_tags($_POST['ucity']); } ?>"tabindex="5"  required></td>
        <td class="sup-label">Fax:</td><td><input pattern="\d{3}[\-]\d{3}[\-]\d{4}" type="tel" title="Enter phone in this format: 000-000-0000"  class="sup-input" name="ufax" value="<?php if(isset($error)){ echo strip_tags($_POST['ufax']); } ?>"tabindex="6" </td>
</tr>
<tr>
	...
	<td class="sup-label">Company:</td><td><input type="text" class="sup-input" name="ucompany" value="<?php if(isset($error)){ echo strip_tags($_POST['ucompany']); } ?>" tabindex="8" required></td>
</tr>
<tr>
	<td class="sup-label">Zip:</td><td><input type="text" class="sup-input" name="uzip" maxlength="250" value="<?php if(isset($error)){ echo strip_tags($_POST['uzip']); } ?>"tabindex="9" required></td>
	<td class="sup-label">Notes:</td><td><input type="text" class="sup-input" title="Please include the name and AA number for any dealerships with which you are associated." name="unotes" placeholder="Please include the name and AA for any dealerships with which you are associated." maxlength="250" value="<?php if(isset($error)){ echo strip_tags($_POST['unotes']); } ?>" tabindex="10" required></td>
</tr>
<tr>
	<td class="sup-label">Choose Username:</td><td><input type="text" placeholder="Username: Must contain at least 6 chararcters." title="Username: must have at least 6 chararcters." name="username" id="username" class="sup-input" placeholder="Username(no spaces)" value="<?php if(isset($error)){echo strip_tags($_POST['username']); }else{echo "";} ?>" tabindex="11"></td>
	

<td class="sup-label">Choose Password</td><td><input type="password" name="password" id="password" class="sup-input" placeholder="Password" tabindex="6" value=""></td>

</tr>
<tr>
<td class="sup-label">&nbsp;</td><td><input class="sup-input" type="hidden" style"display:none"></td>
<td class="sup-label">&nbsp;</td><td><input type="password" name="passwordConfirm" id="passwordConfirm" class="sup-input" placeholder="Confirm Password" tabindex="12"></td>
 </tr>
 
 <tr>
 <td class="sup-label">&nbsp;</td><td><input type="hidden" name="uperms" id="uperms" class="sup-input" placeholder="uperms" value="13"></td> 
 
 <td class="sup-label">&nbsp;</td><td><input type="submit" name="submit"  value="Register" style="margin:0 auto;width:4em;" class="btn btn-primary" tabindex="14"">
 </td>
 </tr>
<tr>
...
$("a[href*='" + location.pathname + "']").addClass("btn-primary");
} else {
